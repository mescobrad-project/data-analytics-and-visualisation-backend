...
class RelevancePropagator:
    ...
    def compute_propagated_relevance(self, layer, relevance):
        ...
        if isinstance(layer,
                      ...
                      #print('got in the MaxPool block')
                      #print('layer', layer)
    ...
    def max_pool_nd_inverse(self, layer_instance, relevance_in):
        ...
        # make sure the relevance is put into the same shape as before. - as before the linear?

        #print('relevance_in shape', relevance_in.shape) # torch.Size([1, 512])
        #print('layer_instance out_shape', layer_instance.out_shape) # torch.Size([1, 512, 10, 9, 9])

        if relevance_in.dim() == 2:
            relevance_in = relevance_in.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(layer_instance.out_shape) # RuntimeError: shape '[1, 512, 10, 9, 9]' is invalid for input of size 512
        else:
