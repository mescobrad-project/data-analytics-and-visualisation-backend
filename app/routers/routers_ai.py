...
from app.utils.utils_ai import train_linear_regression, train_logistic_regression, train_SVC, train_xg
...
async def ai_tabular_ae_training_experiment(
    ...
    print(csv_path, no_of_features, test_size, model_type, iterations, lr, early_stopping_patience)
    results = tabular_run_experiment(workflow_id,
                                     step_id,
                                     run_id,
                                     csv_path,
...
@router.get("/xgboost_create_model")
def xgboost_create_model(
        workflow_id: str,
        step_id: str,
        run_id: str,
        test_size: float,
    ...
    try:
        ...
        test_status = 'Unable to retrieve the dataset'
        data = load_data_from_csv(path_to_storage + "/" + file_name)
        print("0, got dataset", flush=True)
        ...
        test_status = 'Unable to execute regression'
        print("1, before train", flush=True)

        xgboost_model = train_xg(X_train, encoded_y_train)
        print("2, after train", flush=True)
        cla_result = classification_report(encoded_y_test, xgboost_model.predict(X_test), output_dict=True)
        # cla_json = json.dumps(cla_result, indent = 4)
        # print(f"cla_json: {cla_json}")
        # print(cla_result)
        # print(classification_report(encoded_y_test, logistic_model.predict(X_test)))
        ...
        test_status = 'Unable to save the model'
        filename = model_name+'.sav'
        pickle.dump(xgboost_model, open(path_to_storage +'/'+ filename, 'wb'))

        # # TODO: Temporarily we save in both paths - to be available for Load model and also proceed to datalake
        pickle.dump(xgboost_model, open(path_to_storage +'/output/'+ filename, 'wb'))
        # Save the model parameters to a JSON file
        model_params = {
            'independent_variables': independent_variables,
            'dependent_variable': dependent_variable
        ...
        test_status = 'Unable to print model stats'
        score = xgboost_model.score(X_train, encoded_y_train)
        # decision_function = logistic_model.decision_function(X_train)
        # desicion_df=pd.DataFrame(decision_function, columns=['Decision func'])
        # desicion_df.to_csv(path_to_storage + '/output/decision_function.csv', index=False)
        # # print(f"decision_function: {decision_function}")
        # # print(f"type(decision_function): {type(decision_function)}")
        dfslope= pd.DataFrame(xgboost_model.coef_.transpose(), index=independent_variables)
        # print(f"dfslope: {dfslope}")
        ...
        with open(path_to_storage + '/output/info.json', 'r+', encoding='utf-8') as f:
            ...
            new_data = {
                ...
                "test_results": {
                    ...
                    # 'decision_function': desicion_df.to_dict(),
                    'coeff_determination': score,
                    # 'intercept': list(xgboost_model.intercept_),
                    'slope': dfslope.transpose().to_dict(),
                                 }}
            file_data['results'] = new_data
            file_data['Output_datasets'] = [{"file": 'expertsystem/workflow/'+ workflow_id+'/'+ run_id+'/'+
        ...
        return JSONResponse(content={'status': 'Success', 'classification_report': cla_report.to_json(orient='records'),
                                     "coeff_determination": score,
                                     # 'decision_function': desicion_df.to_json(orient='records'),
                                     # 'intercept': float(xgboost_model.intercept_)
                                     'slope': dfslope.transpose().to_json(orient='records')
                                     },
                                    status_code=200)
    except Exception as e:
...
@router.get("/xgboost_load_model")
async def xgboost_load_model(
        workflow_id: str,
        step_id: str,
        run_id: str,
        file_name: str,
    ...
    try:
        ...
        # X = data[test_params['independent_variables']]
        # y = data[test_params['dependent_variable']]
        # # Split dataset into train and test sets
        # X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=random_state,
        #                                                     shuffle=shuffle)
        # if y.dtype == object:
        ...
        X_test = data[test_params['independent_variables']]
        X_test.columns = X_test.columns.astype(str)  # Convert feature names to strings
        col_name = str(test_params['dependent_variable']) + '_predict'
        y_pred = pd.DataFrame(loaded_model.predict(X_test))
        print(f"XGBoost Predict:{loaded_model.coef_}")
        print((X_test.columns))
        # print(loaded_model.feature_names_in_)
        df = pd.DataFrame(loaded_model.coef_, index=test_params['independent_variables'])#,columns=X_test.columns)
        print(f"XGBoost coefs_:{df.transpose()}")
        # print(f"intercept_:{list(loaded_model.intercept_)}")
        data.insert(loc=0, column=col_name, value=y_pred)
        print("XGBoost first column")
        result_dataset = data
        result_dataset.to_csv(path_to_storage + '/output/Dataset_predict.csv', index=False)
        ...
        with open(path_to_storage + '/output/info.json', 'r+', encoding='utf-8') as f:
            ...
            new_data = {
                ...
                "run_id": run_id,
                "step_id": step_id,
                "test_name": 'XGBoost Regression Test Model',
                "test_params": {
                    "Dependent variable": test_params['dependent_variable'],
                    "Independent variables": test_params['independent_variables']
                },
                "test_results": {
                    'coeff_determination': df.transpose().to_dict()
                    # 'intercept': list(loaded_model.intercept_)
                                 }}
            print(new_data)
            file_data['results'] = new_data
        ...
        return JSONResponse(
            content={'status': 'Success',
                     "coeff_determination": df.transpose().to_json(orient='records'),
                     # 'intercept': float(loaded_model.intercept_),
                     'dependent_param':test_params['dependent_variable'], 'independent_params':test_params['independent_variables'],
                     'result_dataset':result_dataset.to_json(orient='records')},
            status_code=200)
